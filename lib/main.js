...
import { initSDKGenerator } from './sdk-generator.js';
...
function parseMarkdown(text) {
  ...
  processed = processed
    ...
    // Images (must come before links to avoid ![alt](url) being matched as a link)
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" style="max-width:100%;margin:1em 0;border-radius:4px;">')
    // Links
...
function renderGuideMarkdown(description, sectionId) {
  let guideHtml = parseMarkdown(description);
  guideHtml = guideHtml.replace(/<h2>(.+?)<\/h2>/g, (match, title) => {
    const headingId = `${sectionId}--${slugifyHeading(title)}`;
    return `<h2 id="${headingId}">${title}</h2>`;
  });
  return guideHtml;
}
...
class DocumentationRenderer {
   ...
   */
  renderNavigation(tagGroups, webhooks, webhookTagNames = new Set()) {
    const nav = document.getElementById('nav');
    let html = '';
    let renderedWebhooks = false;
    ...
    for (const group of tagGroups) {
      ...
      if (webhooks.length > 0 && webhookTagNames.has(group.name) && !renderedWebhooks) {
        html += this.renderWebhooksNavigation(webhooks);
        renderedWebhooks = true;
        if (group.endpoints.length === 0) continue;
      }

      // Description-only tag (no endpoints) — render as expandable guide nav
      if (group.endpoints.length === 0) {
        // Skip tags already rendered by the webhooks section
        if (webhookTagNames.has(group.name)) continue;
        if (group.description && !isMarkdownFileReference(group.description)) {
          // Extract ## headings from markdown description for sub-navigation
          const headings = [];
          const headingRegex = /^##\s+(.+)$/gm;
          let match;
          while ((match = headingRegex.exec(group.description)) !== null) {
          ...
          if (headings.length > 0) {
            ...
            for (const heading of headings) {
              const headingId = `${sectionId}--${heading.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
              html += `
                <a href="#${headingId}" class="nav-item" data-section="${headingId}">
                  <span class="nav-path">${heading}</span>
                </a>
              `;
            }
            html += '</div></div>';
            ...
            html += `
              <a href="#${sectionId}" class="nav-item" data-section="${sectionId}">
                <span class="nav-path">${group.name}</span>
              </a>
            `;
          }
        }
      ...
      if (group.description && hasRichMarkdown(group.description) && !isMarkdownFileReference(group.description)) {
        html += `
          <a href="#${sectionId}" class="nav-item" data-section="${sectionId}">
            <span class="nav-path">Overview</span>
          </a>
    ...
    if (webhooks.length > 0 && !renderedWebhooks) {
      html += this.renderWebhooksNavigation(webhooks);
    }
  ...
  renderWebhooksNavigation(webhooks) {
    let html = `
      ...
      html += `
        <tr>
          ...
          <td data-label="Description">
            <span class="param-description">${parseMarkdown(param.description || schema.description || '')}</span>
            ${enumHtml}
          </td>
        </tr>
      `;
      ...
      html += `
        <div class="schema-property">
          ...
          <div class="schema-property-description">
            ${parseMarkdown(resolvedProp?.description || '')}
            ${enumHtml}
          </div>
        </div>
      `;
      ...
      html += `
        <div class="response-item">
          <div class="response-header">
            ...
            <span class="response-description">${parseMarkdown(response.description || '')}</span>
          </div>
        </div>
      `;
    }
   ...
   */
  renderAllEndpoints(tagGroups, webhookTagNames = new Set(), webhooks = []) {
    ...
    for (const group of tagGroups) {
      ...
      if (webhooks.length > 0 && webhookTagNames.has(group.name) && !renderedWebhooks) {
        html += this.renderWebhooksContent(webhooks, tagGroups);
        renderedWebhooks = true;
        if (group.endpoints.length === 0) continue;
      }

      // Description-only tag (no endpoints) — render as a guide/documentation section
      if (group.endpoints.length === 0) {
        // Skip tags already rendered by the webhooks section
        if (webhookTagNames.has(group.name)) continue;
        if (group.description && !isMarkdownFileReference(group.description)) {
          // Render markdown and add IDs to h2 elements for anchor navigation
          const guideHtml = renderGuideMarkdown(group.description, sectionId);
          html += `
            <div class="tag-section guide-section" id="${sectionId}">
      ...
      // Tag header
      const shouldRenderRichDescription = group.description &&
        hasRichMarkdown(group.description) &&
        !isMarkdownFileReference(group.description);
      const shouldRenderSimpleDescription = group.description &&
      ...
      html += `
        <div class="tag-section" id="${sectionId}">
          <div class="tag-header">
            <h2 class="tag-title">${group.name}</h2>
            ${shouldRenderSimpleDescription ?
              `<div class="tag-description">${parseMarkdown(group.description)}</div>` : ''}
          </div>
          ${shouldRenderRichDescription ?
            `<div class="guide-content tag-guide-content">${renderGuideMarkdown(group.description, sectionId)}</div>` : ''}
        </div>
      `;
   ...
   */
  renderWebhooks(webhooks, tagGroups = []) {
    if (webhooks.length === 0) return;
    const container = document.getElementById('webhooks');
    container.innerHTML = this.renderWebhooksContent(webhooks, tagGroups);
  }

  renderWebhooksContent(webhooks, tagGroups = []) {
    // Use the Webhooks tag description if available, otherwise fall back to default
    const webhookTagNames = new Set(webhooks.flatMap(wh => wh.tags || []));
    const webhookTag = tagGroups.find(g => webhookTagNames.has(g.name) && g.description);
    const description = webhookTag
      ? parseMarkdown(webhookTag.description)
      : '<p>Events sent to your server when data changes</p>';
...
function setupEventListeners() {
  ...
  document.querySelectorAll('.endpoint-section, .webhook-section, .overview-section, .guide-section, .tag-section').forEach(section => {
...
function setupServerSelector(servers, codeGen, renderer, parser, webhookTagNames = new Set()) {
  ...
  select.addEventListener('change', () => {
    ...
    renderer.renderAllEndpoints(tagGroups, webhookTagNames, webhooks);
    setupEventListeners();
...
async function init() {
  try {
    ...
    // Setup spec download link
    const downloadLink = document.getElementById('specDownloadLink');
    if (downloadLink) {
    ...
    // Collect tag names used by webhooks to avoid duplicate rendering
    const webhookTagNames = new Set();
    for (const wh of webhooks) {
      for (const t of (wh.tags || [])) {
        webhookTagNames.add(t);
      }
    }

    // Setup server selector
    if (spec.servers?.length > 0) {
      setupServerSelector(spec.servers, codeGen, renderer, parser, webhookTagNames);
    }

    // Render everything
    renderer.renderNavigation(tagGroups, webhooks, webhookTagNames);
    renderer.renderAllEndpoints(tagGroups, webhookTagNames, webhooks);
    ...
    // Initialize SDK Generator (disabled - not ready for release)
    // const sdkContainer = document.getElementById('sdkGeneratorContainer');
    // if (sdkContainer) {
    //   const { ui: sdkUI } = initSDKGenerator(spec);
    //   sdkContainer.innerHTML = sdkUI.render();
    //   sdkUI.setupEventListeners();
    // }
    
    console.log('Documentation loaded successfully');

    // Handle deep link: scroll to hash target now that content is rendered
