...
function setupOAuth2FromSpec(config) {
      ...
      for (const [name, description] of Object.entries(config.scopes)) {
        scopeHtml += `
          <label class="scope-item">
            ...
            ${description ? `<span class="scope-desc">${escapeHtml(description)}</span>` : ''}
          </label>
        `;
      }
      scopeHtml += '</div>';
  ...
  // If OAuth2 is configured, automatically switch to OAuth2 tab
  if (config.tokenUrl) {
    const oauthTab = document.querySelector('[data-auth-tab="oauth"]');
    if (oauthTab) {
      oauthTab.click();
    }
  }
