    ...
    for (const group of tagGroups) {
      ...
      if (group.endpoints.length === 0) {
        ...
        if (group.description) {
          ...
          let guideHtml = parseMarkdown(group.description);
          guideHtml = guideHtml.replace(/<h2>(.+?)<\/h2>/g, (match, title) => {
            const headingId = `${sectionId}--${title.toLowerCase().replace(/<[^>]+>/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`;
...
function setupEventListeners() {
  ...
  document.querySelectorAll('.endpoint-section, .webhook-section, .overview-section, .guide-section').forEach(section => {
